...
def get_dsets_from_file(file, start, size):
    return get_dsets_from_parent_group(file, start, size)
...
def get_dsets_from_group_directly(study, study_group, start, size):
    datasets = gu.load_dsets_from_group(study_group, TO_QUERY_DSETS, start, size)
    datasets[STUDY_DSET] = gu.create_dataset_from_value(study, study_group, start, size)
    return datasets